...
| `rmq_publish_duration_ms` | Histogram | `service`, `event`, `env` | Publish latency (ms) |
| `rmq_payload_bytes` | Histogram | `service`, `event`, `env` | Message payload size (bytes) |
...
| Metric | Type | Tags | Description |
|--------|------|------|-------------|
| `rmq_consumer_connection_reinit_total` | Counter | `nano_service_name`, `reason` | Connection reinit events |
| `rmq_consumer_connection_reinit_duration_ms` | Timing | `nano_service_name` | Reinit duration (ms) |
...
Safe tags (bounded): `service`, `event`, `error_type`, `retry`, `status`, `env`, `reason`
